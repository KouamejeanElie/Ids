...
<head>
    ...
    <title>IDS - Listing Certificat || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-success"><i class="fa fa-list-alt"></i>&nbsp;Disponibilité de vos certificats</p>
                                <div class="active-member">
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                         @if( $totalCertificat>0)
                                            <thead>
                                                <tr>
                                                    <th>Libéllé</th>
                                                    <th>Statut</th>
                                            ...
                                            <tbody>
                                             @foreach( $listeCertificatDispos as  $listeCertificatDispo)
                                             <tr>
                                                 <td>{{  $listeCertificatDispo->libellecertificat}}</td>
                                                 <td class="badge bg-success" style="color:white;">{{ $listeCertificatDispo->statut}}</td>
                                                 <td><a href="{{ asset('uploads/certificat/'. $listeCertificatDispo->files) }}">Télécharger <i class="fa fa-hand-o-right"></i>&nbsp;{{ $listeCertificatDispo->files }}</a></td>
                                                 <td>{{ $listeCertificatDispo->created_at->format('d:m:Y H:i')}}</td>
                                             </tr>
                                             @endforeach
                                            </tbody>
                                        </table><br />
                                        @else
                                          <p class="alert alert-danger alert-dismissible" style="text-align:center;"><i class="fa fa-info-circle" style="font-size: 24px;"></i>&nbsp;Oops!! Aucun Certificat disponible !
                                                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                             </p>
                                        @endif
                                        {{ $listeCertificatDispos->links() }}
