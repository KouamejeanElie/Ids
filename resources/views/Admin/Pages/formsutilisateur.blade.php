...
<head>
    ...
    <title>IDS - Listing Utilisateurs || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning"><i class="fa fa-list-alt"></i>&nbsp;Utilisateurs IDS</p>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                         @if($users>0)
                                            ...
                                            <tbody>
                                             @foreach($listeTotalUtilisateurs as $listeTotalUtilisateur)
                                                <tr>
                                                    <td>{{ $listeTotalUtilisateur->name }}</td>
                                                    <td>{{ $listeTotalUtilisateur->email }}</td>
                                                    <td>{{ $listeTotalUtilisateur->created_at->format('d:m:Y H:i') }}</td>
                                                     <td>
                                                        <form action="{{ route('path_delete_admin_utilisateur',$listeTotalUtilisateur->id) }}" onsubmit="return confirm('Etes vous sûre ?');" method="POST">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE')}}
                                        ...
                                        @else
                                           <p class="alert alert-danger alert-dismissible" style="text-align:center;"><i class="fa fa-info-circle" style="font-size: 24px;"></i>&nbsp;Oops!! Aucun utilisateur enrégistré ici !
                                                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                             </p>
                                        @endif
                                        {{ $listeTotalUtilisateurs->links() }}
                                    </div>
                                </div>
                            </div>
                        </div>                        
