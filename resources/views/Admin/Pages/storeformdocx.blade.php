...
<head>
    ...
    <title>IDS - Ajout de Docx || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title"><i class="fa fa-plus"></i>&nbsp;Ajouter un document à la plateforme</h4>
                                <div class="basic-form">
                                    <form action="{{ route('path_admin_store_docx') }}" method="POST" enctype="multipart/form-data">
                                        {{ csrf_field() }}
                                        <div class="form-row">
                                            <div class="form-group col-md-6">
                                                <label>Nom du document</label>
                                                <input type="text" name="nomdocument" class="form-control @error('nomdocument') is-invalid @enderror" placeholder="Entrer le nom du document...">
                                                @error('nomdocument')
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning">Liste des documents</p>
                                <div class="active-member">
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                            ...
                                            <tbody>
                                           @foreach( $listeAlldocxs as  $listeAlldocx)

                                           <tr>
                                               <td>{{ $listeAlldocx->id }}</td>
                                               <td>{{ $listeAlldocx->created_at->format('d:m:Y H:i') }}</td>
                                               <td>{{ $listeAlldocx->nomdocument }}</td>
                                               <td><a href="{{ asset('uploads/docx/'. $listeAlldocx->files) }}">Télécharger <i class="fa fa-hand-o-right"></i>&nbsp;{{ $listeAlldocx->files }}</a></td>
                                                   <td>
                                                  <form method="post" action="{{ route('path_delete_admin_store_docx', $listeAlldocx->id) }}" onsubmit="return confirm('Etes-vous sûre!');">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE')}}
