...
<head>
    ...
    <title>IDS - Listing Contact || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning"><i class="fa fa-list-alt"></i>&nbsp;Listing des Contacts</p>
                                         ...
                                         @if($totalContact>0)
                                            ...
                                            <tbody>
                                             @foreach($listeTotalContacts as $listeTotalContact)
                                                <tr>
                                                    <td>{{ $listeTotalContact->identite }}</td>
                                                    <td>{{ $listeTotalContact->contact }}</td>
                                                    <td>{{ $listeTotalContact->mailcontact }}</td>
                                                    <td><textarea class="form-control" readonly>{{ $listeTotalContact->message }}
                                                        </textarea>
                                                    </td>
                                                    <td>{{ $listeTotalContact->created_at->format('d:m:Y H:i') }}</td>
                                                     <td>
                                                        <form action="{{ route('path_delete_admin_contact',$listeTotalContact->id) }}" onsubmit="return confirm('Etes vous sûre ?');" method="POST">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE')}}
