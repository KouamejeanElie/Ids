...
<head>
    ...
    <title>IDS - Listing Rendez vous || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning"><i class="fa fa-list-alt"></i>&nbsp;Listing des Rdv Demandés</p>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                         @if($totalRdv>0)
                                            ...
                                            <tbody>
                                             @foreach($listeTotalRdvs as $listeTotalRdv)
                                                <tr>
                                                    <td>{{ $listeTotalRdv->nom }}</td>
                                                    <td>{{ $listeTotalRdv->mail }}</td>
                                                    <td>{{ $listeTotalRdv->tel }}</td>
                                                    <td>{{ $listeTotalRdv->daterdv->format('d:m:Y') }}
                                                    </td>
                                                    <td>{{ $listeTotalRdv->heurerdv }}</td>
                                                    <td>{{ $listeTotalRdv->services }}</td>
                                                     <td>
                                                        <form action="{{ route('path_delete_admin_rdv',$listeTotalRdv->id) }}" onsubmit="return confirm('Etes vous sûre ?');" method="POST">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE')}}
                                        ...
                                        @else
                                           <p class="alert alert-danger alert-dismissible" style="text-align:center;"><i class="fa fa-info-circle" style="font-size: 24px;"></i>&nbsp;Oops!! Aucun Rendez vous demandé !
                                                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                             </p>
                                        @endif
                                        {{ $listeTotalRdvs->links() }}
                                    </div>
                                </div>
                            </div>
                        </div>                        
