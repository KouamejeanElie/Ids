...
<head>
    ...
    <title>IDS - Valider une Demande de Bourse Utilisateur || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning">Liste des demdandes de bourses/utilisateurs</p>
                                <div class="active-member">
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                            ...
                                            <tbody>
                                             @foreach( $listeAllbourses as  $listeAllbourse)
                                              <tr>
                                                  <td>{{ $listeAllbourse->nom }}</td>
                                                  <td>{{ $listeAllbourse->name }}</td>
                                                  <td>{{ $listeAllbourse->id_user }}</td>
                                                  <td>{{ $listeAllbourse->choixbourse }}</td>
                                                  <td>{{ $listeAllbourse->participation }}</td>
                                                  <td><textarea class="form-control" readonly>{{ $listeAllbourse->message }}
                                                  </textarea></td>
                                                  <td class="badge bg-primary" style="color:white;">{{ $listeAllbourse->statut }}</td>
                                                  <td>
                                                    <a href="{{ route('path_edit_admin_storeformbourse', $listeAllbourse->id) }}">
                                                  <button class="btn btn-block btn-outline-info btn-sm"><i class="fa fa-edit"></i></button>
                                                  </a><br />
                                                  <form method="post" action="{{ route('path_delete_admin_store_bourse', $listeAllbourse->id) }}" onsubmit="return confirm('Etes-vous sûre!');">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE')}}
