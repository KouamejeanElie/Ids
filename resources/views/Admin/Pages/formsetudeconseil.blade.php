...
<head>
    ...
    <title>IDS - Listing Etude & Conseil || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning"><i class="fa fa-list-alt"></i>&nbsp;Listing des demandes d'Etudes & Conseils</p>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                         @if($totalEtude>0)
                                            ...
                                            <tbody>
                                             @foreach($listeTotalEtudes as $listeTotalEtude)
                                                <tr>
                                                    <td>{{ $listeTotalEtude->nom }}</td>
                                                    <td>{{ $listeTotalEtude->emailformation }}</td>
                                                    <td>{{ $listeTotalEtude->contact }}</td>
                                                    <td>{{ $listeTotalEtude->formations }}
                                                    </td>
                                                    <td>{{ $listeTotalEtude->created_at->format('d:m:Y H:i') }}</td>
                                                     <td>
                                                        <form action="{{ route('path_delete_admin_etudeconseil',$listeTotalEtude->id) }}" onsubmit="return confirm('Etes vous sûre ?');" method="POST">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE')}}
