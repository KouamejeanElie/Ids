...
<head>
    ...
    <title>IDS - Listing Intégration web || Tableau de Bord</title>
      <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-warning"><i class="fa fa-list-alt"></i>&nbsp;Listing des demandes de Services d'Intégration Web</p>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                         @if($totalIntegration>0)
                                            ...
                                            <tbody>
                                             @foreach($listeTotalIntegrations as $listeTotalIntegration)
                                                <tr>
                                                    <td>{{ $listeTotalIntegration->nom }}</td>
                                                    <td>{{ $listeTotalIntegration->emailformation }}</td>
                                                    <td>{{ $listeTotalIntegration->contact }}</td>
                                                    <td>{{ $listeTotalIntegration->formations }}
                                                    </td>
                                                    <td>{{ $listeTotalIntegration->created_at->format('d:m:Y H:i') }}</td>
                                                     <td>
                                                        <form action="{{ route('path_delete_admin_integration',$listeTotalIntegration->id) }}" onsubmit="return confirm('Etes vous sûre ?');" method="POST">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE')}}
                                        ...
                                        @else
                                           <p class="alert alert-danger alert-dismissible" style="text-align:center;"><i class="fa fa-info-circle" style="font-size: 24px;"></i>&nbsp;Oops!! Aucune demande ici !
                                                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                             </p>
                                        @endif
                                        {{ $listeTotalIntegrations->links() }}
                                    </div>
                                </div>
                            </div>
                        </div>                        
